 ...
 * @author Ljiljana
 *
 *         DataLoaderSearch is a class that extends DataLoader and is used to
 *         retrieve specific data from the app database.
 */
public class DataLoaderSearch extends DataLoader {
	...
	public void searchData(String event,/* String genre, String musician, */
		...
		try {
			/*
			 * eventsDb = new Select(
			 * "Event.id, eventId, Event.name, description, eventTime, lastUpdate, Event.lat, Event.lng, Event.location"
			 * ).all().from(Event.class)
			 * .join(Location.class).on("Event.location = Location.id"
			 * ).join(EventGenre.class)
			 * .on("Event.id=EventGenre.idEvent").join(Genre
			 * .class).on("EventGenre.idGenre = Genre.id")
			 * .join(EventMusician.class
			 * ).on("Event.id = EventMusician.idEvent").join(Musician.class)
			 * .on(
			 * "EventMusician.idMusician = Musician.id").where("event.name LIKE ?"
			 * , event) .or("eventTime >= ?", dateSince) .or("eventTime <= ?",
			 * dateUntil) .or("genre.name LIKE ?", genre)
			 * .or("musician.name LIKE ?", musician) .or("city LIKE ?",
			 * city).execute();
			 */
			eventsDb = new Select().all().from(Event.class)
					...
					.and("eventTime <= ?", dateUntil).and("city LIKE ?", city)
					.execute();
